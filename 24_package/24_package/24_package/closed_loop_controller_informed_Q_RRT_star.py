...
# Function for Informed RRT* algorithm
def I_Q_RRT_star(start, goal, iterations=3000, search_radius=20, ad=1):
    ...
    # Iterating over the number of iterations for Informed RRT*
    for _ in range(iterations):
        ...
        if new_node:
            ...
            # checking if the goal has been reached by using a threshold of 10 and updating the goal node
...
# Class for Closed Loop Controller Node
class CloseLoopControllerNode(Node):
    # Initializing the node
    def __init__(self, path):
        # Initializing the node
        super().__init__("Closed_loop_informed_Q_RRT_star")
        ...
        self.path_i = 0
        self.linear = PID_Control(Kp = 0.3, Ki = 0, Kd = 0.0)
        self.angular = PID_Control(Kp = 0.6, Ki = 0, Kd = 0.0)
    ...
    def movement_update(self, yaw):
        # Checking if the current pose and path are not None
        ...
        # Calculating the linear and angular velocity using PID controller
...
def main(args=None):
    ...
    tree, goal_node = I_Q_RRT_star(start, goal, iterations=2000, search_radius=20, ad=1)
